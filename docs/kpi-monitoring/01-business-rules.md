...
⚠️ Standar kunjungan (angka pembanding realisasi ini) **masih ASUMSI**, belum dikonfirmasi — lihat paragraf sebelumnya. Sinkron ini cuma ngisi sisi realisasi; formula skor tetap pakai standar asumsi sampai diklarifikasi.
...
**Desain PTID final (2026-08-24)**: pengguna mengonfirmasi populasi FF Hospital aplikasi ini **tersebar di lebih dari satu PT**, bukan satu PTID tunggal seperti yang tadinya diasumsikan. Diverifikasi langsung ke staging DAN production: `AbsPTID=7` mengembalikan data absensi nyata (44 record identik di kedua environment, Juli-Agustus 2026) untuk NIP asli aplikasi ini (`F250028`, role MR); `AbsPTID=1` kosong untuk NIP yang sama, dan 3 NIP lain yang dicoba di staging (`L220012`/MR, `L220015`/MR, `L220022`/ASM) kosong di SELURUH `AbsPTID` yang dicoba (1/2/3/7) untuk rentang Jan-Agu 2026 — tidak bisa dibedakan dari sini apakah itu gap data staging atau memang PT lain. Karena tidak ada cara mengetahui PTID per NIP dari sumber manapun (SIPP tidak punya endpoint "PTID by NIP", dan `get=detailkaryawan` sendiri mewajibkan `KryPTID` sebagai input — bukan output), solusinya: **`ABS_PT_ID_CANDIDATES` di `src/lib/sync/kpiAbsensiSync.ts`** (`[1, 7]`, hardcoded — sama di staging & production, bukan env var karena bukan secret dan tidak berbeda per environment) — job sync mencoba tiap kandidat per NIP atas jendela lookback 6 bulan (bukan cuma bulan target, supaya "tidak ada data bulan ini" tidak disalahartikan sebagai "PTID salah"), kandidat pertama yang punya data DIMENANGKAN dan di-cache ke kolom baru `User.sippAbsPtId` (migration `20260824120000_add_sipp_abs_pt_id`) — sync berikutnya untuk NIP itu langsung pakai nilai cache, tidak probe ulang. NIP yang tidak punya data di kandidat manapun selama 6 bulan dibiarkan `sippAbsPtId = null` (dicoba ulang tiap run, bukan dianggap gagal permanen — bisa jadi karyawan baru).

Daftar kandidat PTID **belum lengkap** — hanya `1` dan `7` yang disebut eksplisit di dokumentasi API. Kalau ada NIP yang tetap kosong di kedua kandidat ini setelah dicoba, kemungkinan populasinya memang mencakup PT lain yang ID-nya belum diketahui — perlu ditambahkan ke `ABS_PT_ID_CANDIDATES` (kode, bukan config) begitu diketahui (dari tim SIPP/HR, bukan ditebak).
