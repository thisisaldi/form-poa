...
- **Dashboard self-view untuk MR** (personil bisa lihat KPI perpanjangan diri sendiri) — **dikonfirmasi 2026-07-30 (user): ini memang fitur yang diinginkan ke depannya, TAPI sengaja belum ditampilkan/di-build sekarang.** Ditahan sampai halaman ADMIN-only ini dianggap matang (sejalan sama alasan `/monitoring` juga masih ADMIN-only — lihat §2). Kalau dikerjakan nanti: MR lihat scorecard diri sendiri read-only (baris dirinya sendiri dari tabel yang sama, bukan halaman terpisah) — cocok dengan role matrix target di §1 (`MR: Diri sendiri (read-only)`), jadi infrastrukturnya (role matrix, scoring engine, data model) sudah siap, tinggal buka akses & bikin view read-only-nya begitu waktunya tiba.
