...
**Revisi 2026-08-26 (setelah feedback pengguna)**:
1. **4 section bertumpuk**: Total → Per NSM → Per SM → Per ASM (bukan cuma ASM). Target=3 outlet **hanya ada di level ASM** ("target ini cuma ada di ASM", dikonfirmasi eksplisit) — di NSM/SM/Total, kolom Target kosong ("-") dan kolom **Achievement** (rename dari "Status", sekarang persentase) menunjukkan **% dari ASM di bawahnya yang mencapai target sendiri-sendiri**, BUKAN jumlah outlet yang di-scale ulang. Contoh: SM dengan 4 ASM, 3 di antaranya sudah ≥3 outlet ber-data-survey → Achievement SM tersebut 75%.
