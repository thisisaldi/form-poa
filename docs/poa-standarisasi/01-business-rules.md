...
|---|---|---|
| 1. Planning Standarisasi | MR (pembuat pengajuan) | Outlet, KPDM Standarisasi + Jabatan, Entertain Estimasi (KPDM), Tipe Standarisasi + Periode, Jumlah Bed RS, Target Penyelesaian, N kartu produk (tiap kartu: produk, **Status Pengajuan (Baru/Perpanjangan — per produk, ditambahkan 2026-08-26 docs/TODO.md #12, dipindah ke level produk 2026-08-27)**: bukan dropdown manual — read-only label, auto-derived server-side (`computeStatusPengajuanMap`, `poaStandarisasi.ts`) dari **OR 4 sinyal** (satu true saja → Perpanjangan, keempat false → Baru): (1) sales `qty > 0` di produk×outlet ini pada salah satu dari **3 bulan kalender terakhir yang sudah selesai** (`OutletSalesMonthly`, bulan berjalan tidak dihitung — **diganti 2026-09-22** dari versi lama yang pakai `OutletSalesHistory.totalSales12Bln` rolling-12-bulan, karena sales 12 bulan lalu dianggap tidak lagi relevan buat "masih aktif sekarang"); (2) produk ini pernah di-submit di POA Standarisasi LAIN di outlet yang sama (`PoaStandarisasiProduk` + `pengajuan.submittedAt not null`); (3) import master data `OutletProductKriteria.kriteriaBaru` untuk produk×outlet ini sudah berlabel `"Produk Sudah Terstandarisasi..."` (match `.startsWith(...)`, sama persis dengan label "Sudah Standarisasi" sidebar); (4) **(baru 2026-09-22)** ada DPL aktif di outlet ini untuk produk ini pada bulan berjalan (`getActiveDplByOutletAction` — kontrak DPL dari `getDiskonByOutlet`, Exodus live dengan fallback `DiskonKontrak`, periode `prdAwal..prdAkhir` mencakup bulan sekarang; rasionalnya, output dari standarisasi itu sendiri adalah DPL, jadi ada DPL aktif = pernah/sedang standarisasi). Dihitung ulang tiap Planning disimpan (`applyPlanningProduk`, fetch DPL SEBELUM `$transaction` supaya HTTP Exodus tidak makan timeout transaksi database), plus live preview di UI via `getStatusPengajuanPreviewAction` sebelum disimpan), HNA SJ/ST, golongan saat ini, estimasi standarisasi [Jumlah Pasien/Resep per Pasien → Qty/Sales computed, **ditambah 2026-09-22: Qty juga dibulatkan ke atas ke Satuan Jual — `estimasiQtyUbPerBulan`, dipakai tampilan & export, lihat 02-data-model.md**], **Skema Diskon/DP** (2026-09-08 — toggle per produk, DISKON pakai `estimasiDiskonPct`/`estimasiDiskonDistributorPct`, DP pakai satu field Rupiah flat `estimasiValueDpRp`, mutually exclusive di UI), Biaya Listing/Entertain per produk, Dokter Klinis wajib). **Update 2026-08-26**: sidebar rekomendasi baru (`RekomendasiSidebar`, docs/TODO.md #15) — Data Survey/Produk Rekomendasi/Sudah Standarisasi/**Historical Sales** (tab ke-4, 2026-09-08), semua agregat per outlet-produk (bukan per dokter); **Sudah Standarisasi diperluas 2026-09-22** untuk juga mencakup DPL aktif (sumber ke-3, selain pengajuan submitted dan label kriteria). |
...
### §3a. Metrik pembanding Perpanjangan (docs/TODO.md #7, 2026-09-07, **dirombak 2026-09-22**)

Informational only, tidak nge-block submit (beda dari warning margin budget §3 yang sudah ada) — tujuannya kasih konteks ke MR: kalau diskon diganti, marginnya bakal jadi berapa dibanding sales & diskon sebelumnya. Satu baris teks inline di bawah baris produk Planning (`PoaStandarisasiWizard.tsx`), cuma muncul untuk `skemaPembayaran === "DISKON"` (bukan DP) DAN produk itu punya diskon live di Exodus (`getMarginWarningBaselineAction` mengembalikan entry — kalau tidak, tidak ada baseline buat dibandingkan, baris tidak muncul).

**Versi 2026-09-07 (superseded)** memakai formula margin flat `100% − diskon PI% − diskon Distributor%` tanpa baseline diskon lama eksplisit. **Versi sekarang** (2026-09-22, user request: "gimana kalau kita ganti diskonnya akan jadi berapa marginnya"):

| Nilai | Formula |
|---|---|
| Sales sebelumnya (`salesLama`) | `base.sales12Bln / 12` — `base` dari `getMarginWarningBaselineAction`, `sales12Bln` = `OutletSalesHistory.totalSales12Bln` |
| Diskon lama (`base.diskonLamaPct`) | `principalPct + distributorPct` dari request diskon Exodus terbaru (`getDiscountsForOutlet`) |
| Diskon baru | `estimasiDiskonPct + estimasiDiskonDistributorPct` (input MR di Planning) |
| **Margin baru** | `MARGIN_CAP_PCT (20%) − (diskon baru − diskon lama)` — margin sebelumnya DIASUMSIKAN = `MARGIN_CAP_PCT` persis (karena sales lama sudah terjadi dengan diskon lama), margin baru bergeser sebesar SELISIH diskon, bukan dihitung ulang dari HPP/HNA riil |
| Growth | `(estTotal / salesLama − 1) × 100`, `estTotal` = total est. sales/bln Planning (semua produk dokter, `totalSales` di kode) |

Tampilan: `sales/bln Rp{salesLama} → Rp{estTotal} (+growth%) · margin {MARGIN_CAP_PCT}% → {marginBaru}% (diskon {diskonLama}% → {diskonBaru}%)`, merah kalau margin baru < `MARGIN_CAP_PCT`.

⚠️ Asumsi "margin sebelumnya = target 20%" belum divalidasi terhadap margin aktual riil (kalau ada sumber data margin sesungguhnya, formula ini perlu diganti) — dicatat sebagai simplifikasi yang disadari, bukan angka final yang dijamin akurat.
