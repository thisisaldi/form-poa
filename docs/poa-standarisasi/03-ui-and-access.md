...
| `RekomendasiSidebar` | ✅ **Diimplementasikan 2026-08-26** (`src/components/poaStandarisasi/RekomendasiSidebar.tsx`), dipasang di `PlanningPhase` (dipakai baik oleh wizard `/poa-standarisasi/[id]` maupun form `/poa-standarisasi/new`, karena keduanya reuse `<PlanningPhase>` yang sama). 3 pill, SEMUA agregat per **outlet-produk** (lintas dokter, BUKAN per satu dokter seperti `PsspSidebar` punya POA Estimasi — sesuai klarifikasi user 2026-08-26, `docs/TODO.md` #15): (1) **Data Survey** — `getSurveyRekomendasiByOutletAggregate(kodePI)` (baru, `customer.ts`), roll-up `SurveyRekomendasi` per `kodeProduk` di seluruh outlet (jumlah dokter, total potensi/bulan, kompetitor gabungan); (2) **Produk Rekomendasi** — reuse `getKriteriaByOutlet(kodePI)` APA ADANYA (fungsi ini SUDAH outlet-level sejak awal, tidak perlu query baru); (3) **Sudah Standarisasi** — merge 2 sumber sampai 2026-09-21, jadi 3 sumber sejak **2026-09-22**: `getStandarisasiProdukByOutletAction(kodePI, pengajuanId)` (`poaStandarisasi.ts`, distinct produk dari pengajuan `PoaStandarisasi` lain yang sudah `submittedAt` di outlet yang sama, exclude pengajuan yang sedang dibuka), `OutletProductKriteria.kriteriaBaru` berlabel "Produk Sudah Terstandarisasi...", **dan DPL aktif** (`getActiveDplByOutletAction`, produk yang punya kontrak DPL live/fallback `DiskonKontrak` mencakup bulan berjalan di outlet ini — detail badge "DPL aktif s/d MM/YYYY"). Ketiga sumber independen, produk yang sudah punya sumber lain tidak ditimpa. Pola UI (fixed-right, edge-tab collapse, pill switcher) di-mirror langsung dari `PsspSidebar` (`LineItemEditor.tsx`) — lihat komentar di file untuk detail. ⚠️ **Belum masuk scope v1 ini** (item #5 di `docs/TODO.md`, masih ambigu terpisah): logika "Produk Fokus PM / Pernah PSSP Dulu Tapi Belum Standarisasi / Sudah Standarisasi Corporate" — tab "Produk Rekomendasi" saat ini murni kriteria paket/kategori existing, belum menerapkan 3 kondisi itu. |
