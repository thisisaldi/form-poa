...
| 3 | Keyword search (nama customer, nama outlet, `code_customer`, `outlet_code`, `id_poa`) | Query param `keyword` baru di `GET /api/poa-doctors` — **scoped DALAM `nip`** yang sudah wajib (dikonfirmasi user: bukan company-wide search, supaya tidak kena constraint `docs/PERFORMANCE.md`). Case-insensitive substring match, OR di kelima field. | ✅ Diimplementasikan |
