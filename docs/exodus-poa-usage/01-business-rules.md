...
### Update 2026-09-22 — ceiling ASM/SM diimplementasikan (bukan cuma NSM/ASD/SD)

`approvalCeiling()` dulu cuma mengenali `"assistant-sales-director"`/`"sales-director"` (naik ke ASD/SD); `"asm"`/`"sm"`/`"nsm"`/`null`/apa pun lainnya semuanya jatuh ke ceiling NSM — jadi kalau Exodus balikin `role: "asm"` atau `role: "sm"`, POA tetap dipaksa naik sampai NSM (Aldi mengonfirmasi ini SALAH, bukan perilaku yang diinginkan). Diperbaiki: `approvalCeiling` sekarang memetakan `"asm"` → ceiling ASM, `"sm"` → ceiling SM juga — dokter fully-approved begitu level itu approve, tidak dipaksa naik lebih jauh.

`approveNsmOrAsd()` (cuma intercept status SUBMITTED_TO_NSM/SUBMITTED_TO_ASD) digeneralisasi jadi `approveAtLevel()`, dipanggil dari `getApproveTransition()` untuk SEMUA status SUBMITTED_TO_ASM/SM/NSM/ASD — setiap step sekarang conditional terhadap ceiling, bukan cuma NSM/ASD. `scripts/testApprovalCeilingChain.ts` diupdate dengan assertion ceiling ASM/SM baru.
