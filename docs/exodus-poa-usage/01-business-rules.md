...
| 3 | Keyword search (nama customer, nama outlet, `code_customer`, `outlet_code`, `id_poa`) | Query param `keyword` baru di `GET /api/poa-doctors`. **DIBALIK 2026-08-27 (hari yang sama)**: awalnya dibangun scoped DALAM `nip` (yang tadinya wajib) supaya tidak kena constraint `docs/PERFORMANCE.md` — tapi user menyadari itu berarti Exodus tidak bisa search by nama kalau belum tahu NIP-nya. Volume data ASLI dicek (bukan diasumsikan lagi): 270 `PoaForm`/6140 `PoaLineItem`/~33 baris NSM-approved per kuartal — jauh di bawah skala insiden `docs/PERFORMANCE.md` #47, dan window kuartal berjalan sudah jadi batas wajib yang tidak tumbuh seiring histori. **`nip` sekarang OPSIONAL** — `keyword` tanpa `nip` men-search company-wide. Diukur company-wide: **~2.6 detik** (di bawah target <3 detik `docs/PERFORMANCE.md`, tapi dekat batas — kandidat pertama untuk dioptimasi kalau volume bertambah signifikan). Minimal satu dari `nip`/`keyword` wajib diisi (400 kalau dua-duanya kosong). Case-insensitive substring match, OR di kelima field. | ✅ Diimplementasikan (dikoreksi 1×) |
