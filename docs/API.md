...
- **Response 200**: array `{ uidPoa, uidCustomer, idPoa, path, periodeKuartal: { startDate, endDate }, approveUntil, usedInExodus, dokter: { kodeCust, namaCust, spesialisasi, kodePI, namaOutlet, outletId, customerId, customerCodeExodus }, estimasi, nilaiPssp, estimasiAktif, nilaiPsspAktif, produk: [{ kodeProduk, namaProduk, estimasi, nilaiPssp, pengaliNilaiR, nilaiR, hna, periodeAwal, lamaPeriode, qtyPerBulan, qtyTotal, productId, principalId, principalName, principalCode, categoryProduct }] }`.
  ...
  - `periodeKuartal` (2026-09-04, rename dari `periode` — breaking, dikoordinasikan dengan Exodus/Budi): start/end tanggal KUARTAL `PoaForm` ini (`quarterDateRange(PoaForm.period)`), SAMA untuk semua dokter/produk dalam 1 kuartal — BUKAN tanggal mulai produk/dokter ini sendiri. Itu ada di `produk[].periodeAwal`/`lamaPeriode` di bawah. Rename ini terjadi karena field lama berulang kali disalahartikan sebagai tanggal mulai per-baris.
  ...
  - `produk[].periodeAwal`/`lamaPeriode` (2026-09-04, tim Exodus): `PoaLineItem.periodeAwal` (`"YYYYMM"`, mentah) / `lamaPeriode` (1/3/6/12) — tanggal mulai & durasi ASLI produk ini, beda dari `periodeKuartal` di atas. Kalau ada baris duplikat `kodeProduk` untuk dokter yang sama, dipakai punya baris PERTAMA (sama seperti `pengaliNilaiR`/`nilaiR`/`hna` — bukan disummed).
