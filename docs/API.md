...
- **Response 200 (`?nip=` diisi)**: array `{ nip, nama, jabatan, target, periode }` — `target` = SUM seluruh GT yang LIVE dipegang subtree nip itu, 1 baris per periode yang punya data. Kosong kalau nip itu (atau subtree-nya) belum punya `MrOutletAssignment` sama sekali.
