...
Target Rupiah bulanan — `?divisi=hospital` (default, boleh dikosongkan) dari `TargetHospitalValue`, `?divisi=non-hospital` dari `TargetNonHospitalValue` (tim project `OMEGA` — spesifikasi lengkap `docs/target-non-hospital-value/`). Satu endpoint dua tabel backing — digabung 2026-09-11 atas permintaan user (sebelumnya sempat jadi route terpisah `/api/target-value-non-hospital` sebentar, digabung balik karena shape query/auth-nya sama persis). Bentuk response tergantung `?nip=` (lihat di bawah, berlaku utk kedua divisi). Diubah 2026-09-02: breakdown default per-GT (bukan per-MR, `namaGT` stabil walau MR-nya pindah/di-vacant-kan, nip tidak), DAN `?nip=` sekarang resolve GT-nya secara LIVE, bukan dari kolom nip* snapshot di tabel target sendiri — kalau GT pindah tangan setelah import, `?nip=` tetap ikut ke pemegang GT SAAT INI tanpa nunggu re-import.
- **Auth**: session login (role **NSM atau ADMIN saja**, role lain 403; NSM selalu dipaksa ke NIP dirinya sendiri — `?nip=` yang dikirim NSM diabaikan) **ATAU** HTTP Basic Auth — kredensial **sama** dengan yang dipakai `/api/poa-doctors` (`PoaDoctorsApiCredential`, diatur ADMIN dari halaman Admin, lihat entri di atas), tidak ada kredensial terpisah untuk endpoint ini. Basic Auth dan session ADMIN bisa query nip manapun. Sama untuk kedua divisi.
- **Query param umum** (semua opsional): `divisi` — `"hospital"` (default) atau `"non-hospital"`, pilih tabel target mana yang dipakai untuk request ini; nilai lain → `400`. `nip` — menentukan rollup siapa yang dikembalikan; role NIP tersebut (`User.role`, harus MR/ASM/SM/NSM) menentukan subtree MR yang di-resolve (MR → dirinya sendiri, ASM/SM/NSM → `getSubordinateMRNips`), lalu GT yang dipegang subtree itu di-SUM. `breakdown` (any value, cuma ngaruh kalau `?nip=` diisi) — return per-GT rows di bawah nip itu instead of 1 angka total. `namaGT` (diabaikan kalau `?nip=` ada/dipaksa session NSM — nip menang) — filter ke SATU GT tertentu, shape sama kayak "get target all".

**`divisi=hospital`** (default):
- `periode` (`YYYYMM`, misalnya `202608`) mempersempit ke 1 bulan, DAN (2026-09-03) menentukan struktur org bulan APA yang dipakai buat resolve GT-nya — `MrOutletAssignment` disync per bulan, jadi target bulan lama pakai GT assignment bulan itu juga, bukan struktur org hari ini. Fallback ke periode sync TERBARU kalau `periode` kosong atau belum pernah disync buat nip ini. Tanpa `periode` sama sekali = selalu pakai struktur terkini.
- GT live resolution: `MrOutletAssignment` + `Outlet.namaGT` (lihat `getCurrentGTsForMrNips` di `src/lib/targetHospitalValue.ts`).
...
- **Response 200 (`?nip=` diisi, tanpa `breakdown`)**: array `{ nip, nama, jabatan, divisi: "hospital", target, periode }`. Kosong kalau nip itu (atau subtree-nya) belum punya `MrOutletAssignment` sama sekali.
- **Response 200 (`?nip=` + `?breakdown=1`)**: array `{ namaGT, nipMR, namaMR, divisi: "hospital", target, periode }` — shape SAMA kayak "get target all" di bawah, tapi cuma GT yang LIVE ada di subtree nip itu.
- **Response 200 (`?nip=` kosong + `?namaGT=` diisi)**: array `{ namaGT, nipMR, namaMR, divisi: "hospital", target, periode }`, cuma 1 GT.
- **Response 200 (`?nip=` dan `?namaGT=` kosong dua-duanya, "get target all")**: array `{ namaGT, nipMR, namaMR, divisi: "hospital", target, periode }` — 1 baris per (GT, periode) COMPANY-WIDE. `namaMR`/`nipMR` di sini MASIH snapshot import, `nipMR` null kalau belum ter-resolve ke User asli.

**`divisi=non-hospital`**:
- **GT live resolution**: tim non-hospital TIDAK punya `Outlet`/`MrOutletAssignment` — pemegang GT saat ini di-resolve langsung dari `User.namaWilayah` (role `MR`, project `OMEGA`, `isActive`), yang memang sudah live (disync `omegaUserSync.ts` tiap sync jalan) — lihat `getCurrentGTsForOmegaMrNips` di `src/lib/targetNonHospitalValue.ts`. `periode` di sini CUMA mempersempit output (tidak ada pilihan struktur org per bulan seperti hospital — tidak ada assignment historis per bulan di sini, cuma snapshot User terkini, selalu current).
...
- **`?kategori=RETAIL`** atau **`?kategori=GROSIR_PBF`** (**opsional**, case-insensitive — dinormalisasi uppercase) — filter tambahan, karena sumber non-hospital punya 2 blok data terpisah per area (Retail vs PBF/Grosir). Dinamai beda dari `?divisi` (yang di sini artinya pilih hospital/non-hospital) biar gak tabrakan. **`kategori` DIOMIT = KEDUA kategori sekaligus dalam 1 response** (tidak ada filter WHERE, bukan berarti kosong/error) — TIDAK perlu 2 panggilan terpisah lalu digabung manual. Efeknya beda per mode: `?nip=` tanpa `breakdown` → `target` = SUM kedua kategori jadi 1 angka; `?nip=&breakdown` / `?namaGT=` / "get all" → baris dari kedua kategori tercampur dalam 1 array, dibedakan lewat field `kategori` di tiap baris (lihat Response di bawah) — pemanggil yang butuh breakdown per kategori tinggal group-by field itu sendiri, bukan query 2x.
- **Tidak ada `normalizeGTName`** — matching `namaGT` literal, karena sumbernya (`User.namaWilayah`) memang sudah sama persis ejaannya dengan target sheet (dikonfirmasi sampling DB 2026-09-11).
- **Response 200**: shape sama seperti hospital, tapi baris breakdown/"get target all" pakai `{ namaGT, divisi: "non-hospital", kategori, nipMR, namaMR, project, target, periode }` (field `kategori` = RETAIL/GROSIR_PBF, menggantikan posisi ASM/SM/NSM columns yang gak ada di sumber non-hospital — lihat `docs/target-non-hospital-value/02-data-model.md`; JANGAN disamakan dengan `divisi` yang di sini SELALU `"non-hospital"`, itu tag hospital-vs-non-hospital, beda konsep). `?nip=` tanpa `breakdown` juga dapet `divisi`+`project`.
  - **`divisi`** (baru 2026-09-13) = `"hospital"` atau `"non-hospital"` di SETIAP baris response endpoint ini (kedua mode) — sekadar penanda baris ini datang dari tabel yang mana, berguna kalau hasil dari dua panggilan (`?divisi=hospital` dan `?divisi=non-hospital`) digabung jadi satu list di sisi pemanggil.
  - **`project`** (baru 2026-09-13, KHUSUS non-hospital) = `User.project` milik `nipMR`/nip yang diquery (`null` kalau `nipMR` belum ter-resolve ke User) — ini SUMBER KEBENARAN POA sendiri untuk keanggotaan OMEGA/non-hospital, di-sync dari Nexus (`omegaUserSync.ts`, `get_employees?project=omega`), **BUKAN** dari `mkt_insight.Struktur_Marketing_PI.Divisi` (pipeline terpisah, bisa lag pas ada migrasi KAM1→OMEGA yang lagi jalan). Baris breakdown/"get target all"/`namaGT` juga SUDAH DIFILTER: baris yang `nipMR`-nya ter-resolve ke User tapi `project`-nya BUKAN `"OMEGA"` (drift setelah import) di-drop, tidak ikut muncul — baris `nipMR` null (GT vacant, belum ter-resolve) tetap muncul.
- **Error**: `400` `nip` NIP tidak didukung/`namaGT` — sama seperti hospital.
- **Error umum (berlaku dua divisi)**: `403` role bukan NSM/ADMIN saat pakai session · `401` Basic Auth gagal/kredensial belum diset · `404` `?nip=` tidak ditemukan · `400` role NIP tersebut bukan MR/ASM/SM/NSM, atau `divisi` bukan `hospital`/`non-hospital`.
