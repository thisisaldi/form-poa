...
- **Response 200**: array `{ uidPoa, uidCustomer, idPoa, nip, path, periode: { startDate, endDate }, kuartal, approveUntil, usedInExodus, dokter: { kodeCust, namaCust, spesialisasi, kodePI, namaOutlet, outletId, customerId, customerCodeExodus }, estimasi, nilaiPssp, estimasiAktif, nilaiPsspAktif, produk: [{ kodeProduk, namaProduk, estimasi, nilaiPssp, pengaliNilaiR, nilaiR, hna, qtyPerBulan, qtyTotal, productId, principalId, principalName, principalCode, categoryProduct }] }`.
  ...
  - `approveUntil` (2026-09-22, diperluas ke ASD/SD): `"ASM"` | `"SM"` | `"NSM"` | `"ASD"` | `"SD"` — highest role yang sudah approve dokter ini, derived dari `PoaDoctorApproval.status` (fallback ke `PoaForm.status` kalau dokter masih DRAFT/REVISI dan belum pernah disubmit). **BUKAN** raw `PoaStatus` enum value (dulu salah didokumentasikan begitu) — lihat `approveUntil()` di `src/lib/poaDoctorsRows.ts`.
  - **Filter "fully approved"** (2026-09-22, diperluas dari NSM-only): response cuma berisi dokter yang statusnya `APPROVED_BY_*` (tidak ada holder lagi yang pending) — sebelumnya di-hardcode `approveUntil === "NSM"` saja, jadi dokter yang ceiling-nya (dari `GET /promotion/v1/pssp/approval-level` Exodus, lihat `docs/exodus-poa-usage/01-business-rules.md` §11) ASM/SM/ASD/SD tidak pernah muncul di sini meski sudah genuinely fully approved menurut Exodus sendiri. Sekarang: dokter fully approved di level MANA PUN (ASM/SM/NSM/ASD/SD) ikut muncul, `approveUntil` menunjukkan level terminal-nya yang sebenarnya — bukan dipaksa NSM.
