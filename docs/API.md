...
- **Response 200**: array `{ uidPoa, uidCustomer, idPoa, path, approveUntil, usedInExodus, dokter: { kodeCust, namaCust, spesialisasi, kodePI, namaOutlet, outletId, customerId, customerCodeExodus }, estimasi, nilaiPssp, estimasiAktif, nilaiPsspAktif, produk: [{ kodeProduk, namaProduk, estimasi, nilaiPssp, pengaliNilaiR, nilaiR, hna, periodeAwal, lamaPeriode, qtyPerBulan, qtyTotal, productId, principalId, principalName, principalCode, categoryProduct }] }`.
  ...
  - `periodeKuartal` (kuartal-level start/end date) — **dihapus 2026-09-04** (breaking, dikoordinasikan dengan Exodus/Budi). Sempat ditambahkan sebentar hari yang sama sebagai rename dari `periode`, tapi ternyata percuma: nilainya SAMA untuk semua row dalam 1 response (endpoint cuma query 1 kuartal per call — caller sudah tahu kuartal mana yang di-query lewat `nip`/`startDate` sendiri). Kalau butuh tanggal mulai per-produk, pakai `produk[].periodeAwal`/`lamaPeriode` di bawah.
