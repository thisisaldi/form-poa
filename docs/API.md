...
- **Query param umum** (semua opsional): `divisi` — `"hospital"` (default) atau `"non-hospital"`, pilih tabel target; nilai lain → `400`. `namaGT` — filter ke SATU GT tertentu (matching lihat per-divisi di bawah). `kodeGT` — filter exact match ke kode GT (lihat catatan `kodeGT` per-divisi di bawah, terutama utk hospital yang mayoritas NULL). `periode` (`YYYYMM`) — persempit ke 1 bulan.
...
- ⚠️ **`kodeGT` MOSTLY NULL** (dikonfirmasi 2026-09-14: ~97 dari ~35000 `Outlet` row yang punya `kodeGT` terisi) — `Outlet.kodeGT` sendiri jarang ke-resolve di pipeline sync-nya (`outletSync.ts` cuma isi kalau resolusi zone-hierarchy Nexus sukses, importer satunya `importStrukturVerifiedKAM.ts` malah gak punya kolom kode GT sama sekali dari sumbernya). Bukan bug di tabel/endpoint ini — gap-nya di data hulu. Jangan andalkan `kodeGT` buat lookup hospital sampai gap ini dibenerin di sumbernya; pakai `namaGT` sebagai key utama untuk hospital.
- **Response 200**: array `{ namaGT, kodeGT, divisi: "hospital", nipMR, namaMR, target, periode }` — 1 baris per (GT, periode), dipersempit ke scope NSM kalau session NSM. `namaMR`/`nipMR` MASIH snapshot import (bukan live), `nipMR` null kalau belum ter-resolve ke User asli.
...
- **Response 200**: array `{ namaGT, kodeGT, divisi: "non-hospital", kategori, nipMR, namaMR, project, target, periode }` (field `kategori` = RETAIL/GROSIR_PBF; JANGAN disamakan dengan `divisi` yang di sini SELALU `"non-hospital"` — itu tag hospital-vs-non-hospital, beda konsep).
  - **`divisi`** (baru 2026-09-13) = `"hospital"` atau `"non-hospital"` di SETIAP baris response endpoint ini — sekadar penanda baris ini datang dari tabel yang mana, berguna kalau hasil dari dua panggilan digabung jadi satu list di sisi pemanggil.
  - **`project`** (baru 2026-09-13) = `User.project` milik `nipMR` (`null` kalau `nipMR` belum ter-resolve) — SUMBER KEBENARAN POA sendiri untuk keanggotaan OMEGA, di-sync dari Nexus (`omegaUserSync.ts`), **BUKAN** dari `mkt_insight.Struktur_Marketing_PI.Divisi` (pipeline terpisah, bisa lag). Baris yang `nipMR`-nya ter-resolve ke User tapi `project`-nya BUKAN `"OMEGA"` (drift setelah import) DI-DROP dari response — baris `nipMR` null (GT vacant) tetap muncul.
