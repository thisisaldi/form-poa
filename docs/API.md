...
- **Response 200**: array `{ uidPoa, idPoa, path, approveUntil, dokter: { kodeCust, namaCust, spesialisasi, kodePI, namaOutlet, outletId, customerId, customerCodeExodus }, estimasi, nilaiPssp, estimasiAktif, nilaiPsspAktif, produk: [{ kodeProduk, namaProduk, estimasi, nilaiPssp, pengaliNilaiR, nilaiR, hna, qtyPerBulan, qtyTotal, productId, principalId, principalName, principalCode, categoryProduct }] }`.
  - `uidPoa` (2026-09-02, rename dari `uidCustomer` — tim Exodus hanya butuh identifier per-baris, bukan per-draft): id salah satu `PoaLineItem` milik dokter ini (dipakai sebagai "anchor item" — sama seperti pola `/poa/[id]/doctor/[itemId]/edit`, yang query ulang semua baris dengan kodePI+namaCust yang sama begitu dibuka, jadi id baris manapun milik dokter ini valid). `PoaForm.id` (draft id lama) **tidak lagi diekspos** sebagai field terpisah — masih tertanam di dalam `path`.
  - `path`: path frontend `/poa/{PoaForm.id}/doctor/{uidPoa}/edit` — halaman detail per-dokter yang sama persis dipakai UI in-app.
...
- **Query param** (keduanya opsional): `nip` — menentukan rollup siapa yang dikembalikan; role NIP tersebut (`User.role`, harus MR/ASM/SM/NSM) menentukan subtree MR yang di-resolve (MR → dirinya sendiri, ASM/SM/NSM → `getSubordinateMRNips`), lalu GT yang LIVE dipegang subtree itu di-SUM. `periode` (`YYYYMM`, misalnya `202608`) mempersempit ke 1 bulan.
- **Response 200 (`?nip=` diisi)**: array `{ nip, nama, jabatan, target, periode }` — `target` = SUM seluruh GT yang LIVE dipegang subtree nip itu, 1 baris per periode yang punya data. Kosong kalau nip itu (atau subtree-nya) belum punya `MrOutletAssignment` sama sekali. Matching GT `Outlet.namaGT` (live) ke `TargetHospitalValue.namaGT` (Excel target) dilakukan setelah dibuang +/-/spasi-nya (`normalizeGTName`, `src/lib/targetHospitalValue.ts`) — dua sumber itu sesekali beda tanda baca buat GT gabungan (mis. "JEMBER + BONDOWOSO" vs "JEMBER BONDOWOSO"). ~19 GT (dari total real, per audit 2026-09-02) masih beda nama TOTAL antara dua sumber (bukan cuma tanda baca) dan otomatis GAK ke-SUM — itu butuh reconcile manual, bukan bug matching.
- **Response 200 (`?nip=` kosong, "get target all")**: array `{ namaGT, nipMR, namaMR, target, periode }` — 1 baris per (GT, periode), `namaGT` adalah unit assignment (1 GT = 1 MR/SPV saat ini). `namaMR`/`nipMR` di sini MASIH snapshot import (siapa yang tercatat pegang GT itu saat Excel terakhir di-import), bukan live seperti jalur `?nip=` — `nipMR` null kalau namanya belum ter-resolve ke User asli (placeholder "VACANT MR ..."/"... (SHADOW)" — lihat catatan resolusi nip di script import).
