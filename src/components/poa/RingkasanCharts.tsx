...
export function RingkasanCompareLegend() {
  return (
...
// Rencana vs Aktif, as a grouped bar chart — matplotlib/seaborn "whitegrid"
// styling: light horizontal gridlines behind flat (unrounded) bars, a thin
// bottom axis line, muted gray tick/label text. Built in raw SVG with an
// explicit viewBox coordinate system (not nested CSS `position:
// relative`/`absolute`, which is what caused the previous version's dots to
// collapse to the wrong position — SVG coordinates can't have that bug
// class, every mark's position is computed directly from the same x/y
// space). Values are direct-labeled above each bar (the actual figure is
// always visible without hovering); a native SVG `<title>` on each bar adds
// a hover tooltip with zero custom-positioning code, since the earlier
// custom HTML tooltip's positioning logic is exactly what broke twice.
export function RingkasanBarPair({ rencanaVal, rencanaLabel, aktifVal, aktifLabel, aktifUnavailable, compact }: BarPairProps) {
  ...
  const max = Math.max(rencanaVal, aktifUnavailable ? 0 : aktifVal, 1);
  ...
  const centerX = W / 2;
  const rencanaX = centerX - barW - gap / 2;
  const aktifX = centerX + gap / 2;
...
export function RingkasanSplitBarPair({ rencanaParts, aktifParts }: SplitBarPairProps) {
  const W = 220;
  const H = 86;
  const marginTop = 16;
  ...
  const allVals = [...rencanaParts.map((p) => p.value), ...(aktifParts?.map((p) => p.value) ?? [])];
  ...
  const totalW = rencanaClusterW + clusterGap + aktifClusterW;
